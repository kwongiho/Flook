		...
		<div id="profile_container" id="profile_container">
			<div id="profile_simple" width="100%">
				<table id="profile_header_tb">
					<tr>
						<td><input type="image"
							src="img/profile_img/likeness.png" width="50px"></td>
						<td><c:if test="${user.userImage != null }">
								<input type="image" id="profile_profileImg"
									src="${user.userImage }" style="width: 50px;">
							</c:if> <c:if test="${user.userImage eq null }">
								<input type="image" id="profile_profileImg"
									src="http://www.blackmores.com.au/~/media/bklau/author/profiledefault.png"
									style="width: 70px;">
							</c:if></td>
						<td><input type="image"
							src="img/profile_img/myWriteRecipe2.png" width="50px"></td>
					</tr>
					<tr>
						<td>받은 좋아요</td>
						<td></td>
					...
					<tr>
						<td>0</td>
						<td><strong><h3>${user.userName}</h3></strong></td>
						<td>14</td>
					</tr>
				</table>
				<div width="100%">
					<%-- <c:choose>
						<c:when test="${sessionScope.userName != user.userName }">
							<input type="button" value="+팔로우" id="profile_modifyBtn"
						...
						<c:when test="${sessionScope.userName == user.userName }">
							...
							<input type="button" value="ㅠㅠ?" id="profile_modifyBtn"
						width="30%" class="myButton" />
						</c:when>
					</c:choose> --%>
					...
					<c:if test="${sessionScope.userName!=user.userName && user.followCode == null}">
						<input type="button" value="+팔로우" id="profile_followBtn"
						width="30%"/>
					</c:if>
					<c:if test="${sessionScope.userName!=user.userName && user.followCode != null}">
						<input type="button" value="✔팔로잉" id="profile_AlreadyFollowBtn"
						width="30%"/>
					</c:if>
					
					<script>
						$(document).ready(function(){
							$("#profile_followBtn").on('click',function(){
								var name = "${user.uUserCode}";
								var email = "${user.userEmail}";
								name += ":follow";
								alert("박세훈님을 팔로우 하였습니다.");
								document.location.href="followCheck?targetProfile="+name+"&userEmail="+email;
							});
							$("#profile_AlreadyFollowBtn").on('click',function(){
								/* alert("클릭 ~!"); */
								var name = "${user.followCode}";
								var email = "${user.userEmail}";
								name += ":following";
								alert("박세훈님을 팔로우 취소 하였습니다.");
								document.location.href="followCheck?targetProfile="+name+"&userEmail="+email;
							});
						});
					</script>
					...
					<%-- <input type="image" id="profile_profileImg"
							src="${sessionScope.userImage }" width="70px">
					<input type="button" value="프로필 수정" id="profile_modifyBtn"
						width="30%" class="myButton" /> --%>
				</div>
			</div>
				...
				<div class="container-fluid">
					<div>
						<ul class="nav navbar-nav" id="profile_navbar-nav">
							<li class="profile_nav"><a href="targetMyGraph?userCode=${user.uUserCode}&userEmail=${user.userEmail}">취향분석표</a></li>
							<li class="profile_nav"><a href="targetMyRecipe?userCode=${user.uUserCode}&userEmail=${user.userEmail}">내가
									작성한 레시피</a></li>
							<li class="profile_nav"><a href="targetMyScrap?userCode=${user.uUserCode}&userEmail=${user.userEmail}">MY스크랩</a></li>
							<li class="profile_nav"><a href="cartList.jsp">장보기 목록</a></li>
							<li class="profile_nav"><a href="FollowingList?userCode=${user.uUserCode}&userEmail=${user.userEmail}">팔로잉</a></li>
							<li class="profile_nav"><a href="FollowerList?userCode=${user.uUserCode}&userEmail=${user.userEmail}">팔로워</a></li>
						</ul>
					</div>
				</div>
				</nav>
